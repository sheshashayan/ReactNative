...
class MainSearchView extends Component {
    constructor(props) {
        ...
        this.noRecordsTxtPress = this.noRecordsTxtPress.bind(this);
        this.hideSearchAndHistoryView = this.hideSearchAndHistoryView.bind(this);
    }
    ...
    onSubmitClick(searchText) {
        ...
        if (searchText.length <= 0 || this.state.inputValue.trim() === "") {
            ...
            var lFunctionTocall = "Serial Search"
            if(searchText.length !== 10)
                lFunctionTocall = "ModelList Search";

            this.setState({willShowHistoryListView: false})
            CheckAndStoreAuthToken(this, lFunctionTocall,"");
    ...
    measureSearchView(layout) {
        ...
        console.log('asdasd');
        var measureSearchViewheight = height + 25;
  ...
  findAnotherStore= () => {
    ...
    hideSearchAndHistoryView() {
        this.setState({willShowSearchResult: false});
        this.setState({willShowHistoryListView: false});
    }
    ...
    ProductCategoryListView(rowId) {
        switch (rowId) {
            case '0': {
                let activityIndicator = new ActivityIndicator();
                activityIndicator.startActivity();
                CheckAndStoreAuthToken(this, "Category");
            }
            break;
            case '1': {
                ...
                CarrierServiceScandit.barcodeScanner(dataResource.ScanditAppKey, (error, serialNumber) => {
                    ...
                    if (error || serialNumber.length <= 0) {
                        ...
                        // Serial search will start automatically as and when app will scan serial bar.
                        this.onSubmitClick(serialNumber)
                    }
                })
            }
            break;
    ...
    render() {
        ...
        if (this.state.willShowSearchResult) {
            searchResultView = (
                <View style={[styles.searchViewContainer,{height: this.state.searchViewHeight,}]}>
                ...
                <View style={[styles.searchViewContainer,{height: this.state.searchViewHeight,}]}>
                    <View style={[styles.searchViewContent]}>
                        <ListView
                            ...
                            /*enableEmptySections={this.state.IsSearchedResultEmpty}*/
                        />
                    </View>
                </View>
            );
            ...
            <TouchableWithoutFeedback onPress={ () => {
                ...
                <View style={styles.mainSearchView}>
                    ...
                    <View style={styles.searchView}>
                        ...
                        <View style={[styles.searchBarUnderline, {
                            height: 40,
                    ...
                    <View
                        ref="mycomponent"
                        onLayout={(event) => this.measureSearchView(event.nativeEvent.layout)}
                        style={[styles.measureSearchView, {flex: 11,}]}
                    >
                        <View style={styles.listTopView}>
                                ...
                                renderRow={(rowData, sectionId, rowId) =>
                                    <View style={styles.rowView}>
                                        <TouchableOpacity style={{flex: 1}}
                                                          onPress={() => this.ProductCategoryListView(rowId)}>
                                            <CellView
                                                title={rowData["Title"]}
                            ...
                            <View style={styles.mapTopView}>
                                ...
                                <TouchableHighlight
                                    ...
                                    style={{
                                        left: (windowWidth / 2 - 125), bottom: 15, width: 250,
    ...
    onPressSearchedresult = (rowData) => {
        console.log(rowData)

        let activityIndicator = new ActivityIndicator();
        activityIndicator.startActivity();
        CheckAndStoreAuthToken(this, "ModelData",rowData.ModelNumber);
    ...
    return (
      <View style={styles.SearchedItemContainer}>
        <TouchableOpacity onPress={() => this.onPressSearchedresult(data)} style={styles.SearchedItems}>
          <View style={styles.rowSearchedItem}>
            <View style={styles.searchContentLeft}>
              <Text style={styles.SearchedItemDefault}>{data.ModelDescription}</Text>
              <Text style={styles.SearchedItemDefault}>{data.ModelApplication}</Text>
              <Text style={styles.SearchedItemDefault}>Model #: {data.ModelNumber}</Text>
            </View>
